...
/****************************************
* RIPPED FROM Source/Editor/ComponentVisualizers/Private/SplineComponentVisualizer.cpp
***************************************/
...
IMPLEMENT_HIT_PROXY(HSCSplineVisProxy, HComponentVisProxy);
IMPLEMENT_HIT_PROXY(HSCSplineKeyProxy, HSCSplineVisProxy);
IMPLEMENT_HIT_PROXY(HSCSplineSegmentProxy, HSCSplineVisProxy);
IMPLEMENT_HIT_PROXY(HSCSplineTangentHandleProxy, HSCSplineVisProxy);

#define LOCTEXT_NAMESPACE "ILLFonic.F13.SCCameraSplineComponentVisualizer"


/** Define commands for the spline component visualizer */
class FSCCameraSplineComponentVisualizerCommands : public TCommands<FSCCameraSplineComponentVisualizerCommands>
{
public:
	FSCCameraSplineComponentVisualizerCommands() : TCommands <FSCCameraSplineComponentVisualizerCommands>
		(
			"CameraSplineComponentVisualizer",	// Context name for fast lookup
			LOCTEXT("CameraSplineComponentVisualizer", "Camera Spline Component Visualizer"),	// Localized context name for displaying
			NAME_None,	// Parent
...
FSCCameraSplineComponentVisualizer::FSCCameraSplineComponentVisualizer()
...
{
	FSCCameraSplineComponentVisualizerCommands::Register();
...
void FSCCameraSplineComponentVisualizer::OnRegister()
{
	const auto& Commands = FSCCameraSplineComponentVisualizerCommands::Get();

	SplineComponentVisualizerActions->MapAction(
		Commands.DeleteKey,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnDeleteKey),
		FCanExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::CanDeleteKey));

	SplineComponentVisualizerActions->MapAction(
		Commands.DuplicateKey,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnDuplicateKey),
		FCanExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::IsKeySelectionValid));

	SplineComponentVisualizerActions->MapAction(
		Commands.AddKey,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnAddKey),
		FCanExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::CanAddKey));

	SplineComponentVisualizerActions->MapAction(
		Commands.ResetToUnclampedTangent,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnResetToAutomaticTangent, CIM_CurveAuto),
		FCanExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::CanResetToAutomaticTangent, CIM_CurveAuto));

	SplineComponentVisualizerActions->MapAction(
		Commands.ResetToClampedTangent,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnResetToAutomaticTangent, CIM_CurveAutoClamped),
		FCanExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::CanResetToAutomaticTangent, CIM_CurveAutoClamped));

	SplineComponentVisualizerActions->MapAction(
		Commands.SetKeyToCurve,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnSetKeyType, CIM_CurveAuto),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &FSCCameraSplineComponentVisualizer::IsKeyTypeSet, CIM_CurveAuto));

	SplineComponentVisualizerActions->MapAction(
		Commands.SetKeyToLinear,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnSetKeyType, CIM_Linear),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &FSCCameraSplineComponentVisualizer::IsKeyTypeSet, CIM_Linear));

	SplineComponentVisualizerActions->MapAction(
		Commands.SetKeyToConstant,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnSetKeyType, CIM_Constant),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &FSCCameraSplineComponentVisualizer::IsKeyTypeSet, CIM_Constant));

	SplineComponentVisualizerActions->MapAction(
		Commands.VisualizeRollAndScale,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnSetVisualizeRollAndScale),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &FSCCameraSplineComponentVisualizer::IsVisualizingRollAndScale));

	SplineComponentVisualizerActions->MapAction(
		Commands.DiscontinuousSpline,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnSetDiscontinuousSpline),
		FCanExecuteAction(),
		FIsActionChecked::CreateSP(this, &FSCCameraSplineComponentVisualizer::IsDiscontinuousSpline));

	SplineComponentVisualizerActions->MapAction(
		Commands.ResetToDefault,
		FExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::OnResetToDefault),
		FCanExecuteAction::CreateSP(this, &FSCCameraSplineComponentVisualizer::CanResetToDefault));
}

FSCCameraSplineComponentVisualizer::~FSCCameraSplineComponentVisualizer()
{
	FSCCameraSplineComponentVisualizerCommands::Unregister();
}
...
void FSCCameraSplineComponentVisualizer::DrawVisualization(const UActorComponent* Component, const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	if (const USCCameraSplineComponent* SplineComp = Cast<const USCCameraSplineComponent>(Component))
	{
		const FInterpCurveVector& SplineInfo = SplineComp->GetSplinePointsPosition();
		const FInterpCurveQuat& SplineRotation = SplineComp->GetSplinePointsRotation();
		const FInterpCurveFloat& SplineFOV = SplineComp->GetSplinePointsFOV();
		const USCCameraSplineComponent* EditedSplineComp = GetEditedSplineComponent();

		const USCCameraSplineComponent* Archetype = CastChecked<USCCameraSplineComponent>(SplineComp->GetArchetype());
		...
		if (SplineComp == EditedSplineComp)
		{
			for (int32 SelectedKey : SelectedKeys)
			{
				if (SplineInfo.Points[SelectedKey].IsCurveKey())
				{
					...
					{
						PDI->SetHitProxy(new HSCSplineTangentHandleProxy(Component, SelectedKey, false));
					}
					PDI->DrawPoint(Location + LeaveTangent, SelectedColor, TangentHandleSize, SDPG_Foreground);

					if (bIsSplineEditable)
					{
						PDI->SetHitProxy(new HSCSplineTangentHandleProxy(Component, SelectedKey, true));
					}
		...
		for (int32 KeyIdx = 0; KeyIdx < NumSegments + 1; KeyIdx++)
		{
			...
			if (KeyIdx < NumPoints)
			{
				...
				if (bIsSplineEditable)
				{
					PDI->SetHitProxy(new HSCSplineKeyProxy(Component, KeyIdx));
				}
				// Draw first key as blue for point of reference.
				if (KeyIdx == 0)
					PDI->DrawPoint(NewKeyPos, FColor::Blue, GrabHandleSize, SDPG_Foreground);
				else
					PDI->DrawPoint(NewKeyPos, KeyColor, GrabHandleSize, SDPG_Foreground);
				PDI->SetHitProxy(NULL);

				//Visualize rotation and FOV for each point
				const FRotator Rot = SplineComp->GetRotationAtSplineInputKey(KeyIdx, ESplineCoordinateSpace::World);
				const float FOVScale = 30.f * (90.f / SplineFOV.Points[KeyIdx].OutVal);
				PDI->DrawLine(NewKeyPos, NewKeyPos + Rot.Vector() * FOVScale, KeyColor, SDPG_Foreground);
			}
			...
			if (KeyIdx > 0)
			{
				...
				if (bIsSplineEditable)
				{
					PDI->SetHitProxy(new HSCSplineSegmentProxy(Component, KeyIdx - 1));
				}
...
void FSCCameraSplineComponentVisualizer::ChangeSelectionState(int32 Index, bool bIsCtrlHeld)
{
	if (Index == INDEX_NONE)
	{
		SelectedKeys.Empty();
...
bool FSCCameraSplineComponentVisualizer::VisProxyHandleClick(FEditorViewportClient* InViewportClient, HComponentVisProxy* VisProxy, const FViewportClick& Click)
{
	if(VisProxy && VisProxy->Component.IsValid())
	{
		const USCCameraSplineComponent* SplineComp = CastChecked<const USCCameraSplineComponent>(VisProxy->Component.Get());
		...
		if(SplineCompPropName.IsValid())
		{
			...
			if (VisProxy->IsA(HSCSplineKeyProxy::StaticGetType()))
			{
				// Control point clicked

				HSCSplineKeyProxy* KeyProxy = (HSCSplineKeyProxy*)VisProxy;
			...
			else if (VisProxy->IsA(HSCSplineSegmentProxy::StaticGetType()))
			{
				...
				HSCSplineSegmentProxy* SegmentProxy = (HSCSplineSegmentProxy*)VisProxy;
			...
			else if (VisProxy->IsA(HSCSplineTangentHandleProxy::StaticGetType()))
			{
				// Tangent handle clicked

				HSCSplineTangentHandleProxy* KeyProxy = (HSCSplineTangentHandleProxy*)VisProxy;
...
USCCameraSplineComponent* FSCCameraSplineComponentVisualizer::GetEditedSplineComponent() const
{
	return Cast<USCCameraSplineComponent>(GetComponentFromPropertyName(SplineOwningActor.Get(), SplineCompPropName));
}


bool FSCCameraSplineComponentVisualizer::GetWidgetLocation(const FEditorViewportClient* ViewportClient, FVector& OutLocation) const
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	if (SplineComp != nullptr)
	{
		...
		if (SelectedTangentHandle != INDEX_NONE)
		{
			...
			{
				OutLocation = SplineComp->GetComponentToWorld().TransformPosition(Point.OutVal + Point.LeaveTangent);
			}
			else if (SelectedTangentHandleType == ESelectedTangentHandle::Arrive)
			{
				OutLocation = SplineComp->GetComponentToWorld().TransformPosition(Point.OutVal - Point.ArriveTangent);
			}
		...
		else if (LastKeyIndexSelected != INDEX_NONE)
		{
			...
			OutLocation = SplineComp->GetComponentToWorld().TransformPosition(Point.OutVal);
			return true;
		}
	}
...
bool FSCCameraSplineComponentVisualizer::GetCustomInputCoordinateSystem(const FEditorViewportClient* ViewportClient, FMatrix& OutMatrix) const
{
...
bool FSCCameraSplineComponentVisualizer::IsVisualizingArchetype() const
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
...
bool FSCCameraSplineComponentVisualizer::HandleInputDelta(FEditorViewportClient* ViewportClient, FViewport* Viewport, FVector& DeltaTranslate, FRotator& DeltaRotate, FVector& DeltaScale)
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	if (SplineComp != nullptr)
	{
		...
		if (SelectedTangentHandle != INDEX_NONE)
		{
			...
			if (!DeltaTranslate.IsZero())
			{
				...
				if (SplineComp->bAllowDiscontinuousSpline)
				{
					if (SelectedTangentHandleType == ESelectedTangentHandle::Leave)
					{
						EditedPoint.LeaveTangent += SplineComp->GetComponentToWorld().InverseTransformVector(DeltaTranslate);
					}
					else
					{
						EditedPoint.ArriveTangent += SplineComp->GetComponentToWorld().InverseTransformVector(-DeltaTranslate);
					}
				}
				else
				{
					const FVector Delta = (SelectedTangentHandleType == ESelectedTangentHandle::Leave) ? DeltaTranslate : -DeltaTranslate;
					const FVector Tangent = EditedPoint.LeaveTangent + SplineComp->GetComponentToWorld().InverseTransformVector(Delta);
		...
		else
		{
			...
			for (int32 SelectedKeyIndex : SelectedKeys)
			{
				...
				if (!DeltaTranslate.IsZero())
				{
					...
					const FVector CurrentWorldPos = SplineComp->GetComponentToWorld().TransformPosition(EditedPoint.OutVal);
					// Move in world space
					const FVector NewWorldPos = CurrentWorldPos + DeltaTranslate;
					// Convert back to local space
					EditedPoint.OutVal = SplineComp->GetComponentToWorld().InverseTransformPosition(NewWorldPos);
				}
				...
				if (!DeltaRotate.IsZero())
				{
					...
					FVector NewTangent = SplineComp->GetComponentToWorld().GetRotation().RotateVector(EditedPoint.LeaveTangent); // convert local-space tangent vector to world-space
					NewTangent = DeltaRotate.RotateVector(NewTangent); // apply world-space delta rotation to world-space tangent
					NewTangent = SplineComp->GetComponentToWorld().GetRotation().Inverse().RotateVector(NewTangent); // convert world-space tangent vector back into local-space
					...
					FQuat NewRot = SplineComp->GetComponentToWorld().GetRotation() * EditedRotPoint.OutVal; // convert local-space rotation to world-space
					NewRot = DeltaRotate.Quaternion() * NewRot; // apply world-space rotation
					NewRot = SplineComp->GetComponentToWorld().GetRotation().Inverse() * NewRot; // convert world-space rotation to local-space
...
bool FSCCameraSplineComponentVisualizer::HandleInputKey(FEditorViewportClient* ViewportClient, FViewport* Viewport, FKey Key, EInputEvent Event)
{
	bool bHandled = false;
...
void FSCCameraSplineComponentVisualizer::EndEditing()
{
...
void FSCCameraSplineComponentVisualizer::OnDuplicateKey()
{
...
void FSCCameraSplineComponentVisualizer::DuplicateKey()
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	...
	FInterpCurveFloat& SplineFOV = SplineComp->SplineCurves.FOVCurve;
	FInterpCurveFloat& SplineHoldTime = SplineComp->SplineCurves.HoldTimeCurve;
	FInterpCurveFloat& SplineTimeToNext = SplineComp->SplineCurves.TimeToNextCurve;
	FInterpCurveFloat& SplineTimeAtPoint = SplineComp->SplineCurves.TimeAtPointCurve;
	...
	for (int32 SelectedKeyIndex : SelectedKeysSorted)
	{
		...
		SplineFOV.Points.Insert(FInterpCurvePoint<float>(SplineFOV.Points[SelectedKeyIndex]), SelectedKeyIndex);
		SplineHoldTime.Points.Insert(FInterpCurvePoint<float>(SplineHoldTime.Points[SelectedKeyIndex]), SelectedKeyIndex);
		SplineTimeToNext.Points.Insert(FInterpCurvePoint<float>(SplineTimeToNext.Points[SelectedKeyIndex]), SelectedKeyIndex);

		FInterpCurvePoint<float> Point = SplineTimeAtPoint.Points[SelectedKeyIndex];
		if (SelectedKeyIndex == SplineTimeAtPoint.Points.Num() - 1)
		{
			SplineTimeAtPoint.Points[SelectedKeyIndex].OutVal += 1.0f;
		}
		else
		{
			float NextTime = SplineTimeAtPoint.Points[SelectedKeyIndex + 1].OutVal;
			SplineTimeAtPoint.Points[SelectedKeyIndex].OutVal = Point.OutVal + (NextTime - Point.OutVal) * 0.5f;
		}
		SplineTimeAtPoint.Points.Insert(Point, SelectedKeyIndex);
		...
		for (int Index = SelectedKeyIndex + 1; Index < SplinePosition.Points.Num(); Index++)
		{
			...
			SplineFOV.Points[Index].InVal += 1.0f;
			SplineHoldTime.Points[Index].InVal += 1.0f;
			SplineTimeToNext.Points[Index].InVal += 1.0f;
			SplineTimeAtPoint.Points[Index].InVal += 1.0f;
		}
	}
...
bool FSCCameraSplineComponentVisualizer::CanAddKey() const
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	if (SplineComp == nullptr)
	{
...
void FSCCameraSplineComponentVisualizer::OnAddKey()
{
	...
	FInterpCurveQuat& SplineRotation = SplineComp->GetSplinePointsRotation();
	FInterpCurveVector& SplineScale = SplineComp->GetSplinePointsScale();
	FInterpCurveFloat& SplineFOV = SplineComp->GetSplinePointsFOV();
	FInterpCurveFloat& SplineHoldTime = SplineComp->GetSplinePointsHoldTime();
	FInterpCurveFloat& SplineTimeToNext = SplineComp->GetSplinePointsTimeToNext();
	FInterpCurveFloat& SplineTimeAtPoint = SplineComp->GetSplinePointsTimeAtPoint();

	FInterpCurvePoint<FVector> NewPoint(
		SelectedSegmentIndex,
		SplineComp->GetComponentToWorld().InverseTransformPosition(SelectedSplinePosition),
		FVector::ZeroVector,
		FVector::ZeroVector,
		CIM_CurveAuto);
	...
	FInterpCurvePoint<float> NewFOVPoint(
		SelectedSegmentIndex,
		90.f,
		0.f,
		0.f,
	...
	FInterpCurvePoint<float> NewHoldTimePoint(
		SelectedSegmentIndex,
		0.f,
		0.f,
		0.f,
	...
	FInterpCurvePoint<float> NewTimeToNextPoint(
		SelectedSegmentIndex,
		1.f,
		0.f,
		0.f,
	...
	float newTime = SplineTimeAtPoint.Points[SelectedSegmentIndex].OutVal;
	if (SelectedSegmentIndex == SplineTimeAtPoint.Points.Num() - 1)
	{
		newTime += 1.f;
	}
	else
	{
		float TimeAtNext = SplineTimeAtPoint.Points[SelectedSegmentIndex + 1].OutVal;
		newTime = newTime + (TimeAtNext - newTime) * 0.5f;
	}

	FInterpCurvePoint<float> NewTimeAtPoint(
		SelectedSegmentIndex,
		newTime,
		0.f,
		0.f,
	...
	SplineFOV.Points.Insert(NewFOVPoint, SelectedSegmentIndex + 1);
	SplineHoldTime.Points.Insert(NewHoldTimePoint, SelectedSegmentIndex + 1);
	SplineTimeToNext.Points.Insert(NewTimeToNextPoint, SelectedSegmentIndex + 1);
	SplineTimeAtPoint.Points.Insert(NewTimeAtPoint, SelectedSegmentIndex + 1);
...
void FSCCameraSplineComponentVisualizer::OnDeleteKey()
{
	...
	for (int32 SelectedKeyIndex : SelectedKeysSorted)
	{
		...
		SplineFOV.Points.RemoveAt(SelectedKeyIndex);
		SplineHoldTime.Points.RemoveAt(SelectedKeyIndex);
		SplineTimeToNext.Points.RemoveAt(SelectedKeyIndex);
		SplineTimeAtPoint.Points.RemoveAt(SelectedKeyIndex);
...
bool FSCCameraSplineComponentVisualizer::CanDeleteKey() const
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
...
bool FSCCameraSplineComponentVisualizer::IsKeySelectionValid() const
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	return (SplineComp != nullptr &&
		SelectedKeys.Num() > 0 &&
...
void FSCCameraSplineComponentVisualizer::OnResetToAutomaticTangent(EInterpCurveMode Mode)
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	if (SplineComp != nullptr)
	{
...
bool FSCCameraSplineComponentVisualizer::CanResetToAutomaticTangent(EInterpCurveMode Mode) const
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
...
void FSCCameraSplineComponentVisualizer::OnSetKeyType(EInterpCurveMode Mode)
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	if (SplineComp != nullptr)
	{
...
bool FSCCameraSplineComponentVisualizer::IsKeyTypeSet(EInterpCurveMode Mode) const
{
...
void FSCCameraSplineComponentVisualizer::OnSetVisualizeRollAndScale()
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	check(SplineComp != nullptr);
	...
	NotifyPropertyModified(SplineComp, FindField<UProperty>(USCCameraSplineComponent::StaticClass(), GET_MEMBER_NAME_CHECKED(USCCameraSplineComponent, bShouldVisualizeScale)));

	GEditor->RedrawLevelEditingViewports(true);
}


bool FSCCameraSplineComponentVisualizer::IsVisualizingRollAndScale() const
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
...
void FSCCameraSplineComponentVisualizer::OnSetDiscontinuousSpline()
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	check(SplineComp != nullptr);
	...
	Properties.Add(FindField<UProperty>(USCCameraSplineComponent::StaticClass(), GET_MEMBER_NAME_CHECKED(USCCameraSplineComponent, bAllowDiscontinuousSpline)));
...
bool FSCCameraSplineComponentVisualizer::IsDiscontinuousSpline() const
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
...
void FSCCameraSplineComponentVisualizer::OnResetToDefault()
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	check(SplineComp != nullptr);
...
bool FSCCameraSplineComponentVisualizer::CanResetToDefault() const
{
	USCCameraSplineComponent* SplineComp = GetEditedSplineComponent();
	check(SplineComp != nullptr);
	return SplineComp->SplineCurves != CastChecked<USCCameraSplineComponent>(SplineComp->GetArchetype())->SplineCurves;
	//	return SplineComp->bSplineHasBeenEdited;
}


TSharedPtr<SWidget> FSCCameraSplineComponentVisualizer::GenerateContextMenu() const
{
	FMenuBuilder MenuBuilder(true, SplineComponentVisualizerActions);
	{
		MenuBuilder.BeginSection("SplinePointEdit", LOCTEXT("SplinePoint", "Spline Point"));
		{
			...
			{
				MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().AddKey);
			}
			else if (LastKeyIndexSelected != INDEX_NONE)
			{
				MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().DeleteKey);
				MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().DuplicateKey);

				MenuBuilder.AddSubMenu(
					LOCTEXT("SplinePointType", "Spline Point Type"),
					LOCTEXT("KeyTypeTooltip", "Define the type of the spline point."),
					FNewMenuDelegate::CreateSP(this, &FSCCameraSplineComponentVisualizer::GenerateSplinePointTypeSubMenu));
					...
					for (int32 SelectedKeyIndex : SelectedKeys)
					{
						...
						if (Point.IsCurveKey())
						{
							MenuBuilder.AddSubMenu(
								...
								FNewMenuDelegate::CreateSP(this, &FSCCameraSplineComponentVisualizer::GenerateTangentTypeSubMenu));
							break;
						}
					}
				}
		...
		{
			MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().ResetToDefault);
		}
		MenuBuilder.EndSection();

		MenuBuilder.BeginSection("Visualization", LOCTEXT("Visualization", "Visualization"));
		{
			MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().VisualizeRollAndScale);
			MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().DiscontinuousSpline);
		}
		MenuBuilder.EndSection();
	}
...
void FSCCameraSplineComponentVisualizer::GenerateSplinePointTypeSubMenu(FMenuBuilder& MenuBuilder) const
{
	MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().SetKeyToCurve);
	MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().SetKeyToLinear);
	MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().SetKeyToConstant);
}


void FSCCameraSplineComponentVisualizer::GenerateTangentTypeSubMenu(FMenuBuilder& MenuBuilder) const
{
	MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().ResetToUnclampedTangent);
	MenuBuilder.AddMenuEntry(FSCCameraSplineComponentVisualizerCommands::Get().ResetToClampedTangent);
}
