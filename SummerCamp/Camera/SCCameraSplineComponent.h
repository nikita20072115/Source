...
/****************************************
 * RIPPED FROM Runtime/Engine/Classes/Components/SplineComponent.h
 ***************************************/
...
#include "SCCameraSplineComponent.generated.h"

class USCSplineCamera;

USTRUCT()
struct FSCCameraSplineCurves : public FSplineCurves
{
	GENERATED_BODY()

	/** Spline built from FOV data */
	UPROPERTY()
	FInterpCurveFloat FOVCurve;

	/** Spline built from Hold Time data */
	UPROPERTY()
	FInterpCurveFloat HoldTimeCurve;

	/** Spline built from Time to Next data */
	UPROPERTY()
	FInterpCurveFloat TimeToNextCurve;

	/** Spline built from the time at point data */
	UPROPERTY()
	FInterpCurveFloat TimeAtPointCurve;

	bool operator==(const FSCCameraSplineCurves& Other) const
	{
		return Position == Other.Position
			&& Rotation == Other.Rotation
			&& Scale == Other.Scale
			&& FOVCurve == Other.FOVCurve
			&& HoldTimeCurve == Other.HoldTimeCurve
			&& TimeToNextCurve == Other.TimeToNextCurve
			&& TimeAtPointCurve == Other.TimeAtPointCurve;
	}
...
struct FSCCameraSplinePoint : public FSplinePoint
{
	GENERATED_BODY()

	/** Field of view for the current point */
	UPROPERTY(EditAnywhere, Category = SplinePoint)
	float FOV;

	/** Amount of time to hold the camera at the current point */
	UPROPERTY(EditAnywhere, Category = SplinePoint)
	float HoldTime;

	/** Amount of time to take while animating to the next point */
	UPROPERTY(EditAnywhere, Category = SplinePoint)
	float TimeToNext;
	...
	/** Default constructor */
	FSCCameraSplinePoint()
		: FSplinePoint(), FOV(90.f), HoldTime(0.f), TimeToNext(1.f), TimeAtPoint(0.f)
	{}

	/** Constructor taking a point position */
	FSCCameraSplinePoint(float InInputKey, const FVector& InPosition)
		: FSplinePoint(InInputKey, InPosition), FOV(90.f), HoldTime(0.f), TimeToNext(1.f), TimeAtPoint(0.f)
	{}

	/** Constructor taking a point position and type, and optionally rotation and scale */
	FSCCameraSplinePoint(float InInputKey, const FVector& InPosition, ESplinePointType::Type InType, const FRotator& InRotation = FRotator(0.0f), const FVector& InScale = FVector(1.0f))
		: FSplinePoint(InInputKey, InPosition, InType, InRotation, InScale), FOV(90.f), HoldTime(0.f), TimeToNext(1.f), TimeAtPoint(0.f)
	{}

	/** Constructor taking a point position and tangent, and optionally rotation and scale */
	FSCCameraSplinePoint(float InInputKey,
		...
		: FSplinePoint(InInputKey, InPosition, InArriveTangent, InLeaveTangent, InRotation, InScale, InType), FOV(90.f), HoldTime(0.f), TimeToNext(1.f), TimeAtPoint(0.f)
	{}
};
...
class SUMMERCAMP_API USCCameraSplineComponent : public UPrimitiveComponent
{
...
public:
	...
	FInterpCurveFloat& GetSplinePointsFOV() { return SplineCurves.FOVCurve; }
	const FInterpCurveFloat& GetSplinePointsFOV() const { return SplineCurves.FOVCurve; }
	FInterpCurveFloat& GetSplinePointsHoldTime() { return SplineCurves.HoldTimeCurve; }
	const FInterpCurveFloat& GetSplinePointsHoldTime() const { return SplineCurves.HoldTimeCurve; }
	FInterpCurveFloat& GetSplinePointsTimeToNext() { return SplineCurves.TimeToNextCurve; }
	const FInterpCurveFloat& GetSplinePointsTimeToNext() const { return SplineCurves.TimeToNextCurve; }
	FInterpCurveFloat& GetSplinePointsTimeAtPoint() { return SplineCurves.TimeAtPointCurve; }
	const FInterpCurveFloat& GetSplinePointsTimeAtPoint() const { return SplineCurves.TimeAtPointCurve; }


	void ApplyComponentInstanceData(class FSCSplineInstanceData* ComponentInstanceData, const bool bPostUCS);
	...
	/** Get Field of View at the provided input key value */
	float GetFOVAtSplineInputKey(float InKey) const;

	/** Get Hold Time at the provided input key value */
	float GetHoldTimeAtSplineInputKey(float InKey) const;

	/** Get Time to Next at the provided input key value */
	float GetTimeToNextAtSplineInputKey(float InKey) const;
	...
	UFUNCTION()
	void ResetSplineDefaults();

	/** Clears all the points in the spline */
	UFUNCTION(BlueprintCallable, Category = Spline)
	void ClearSplinePoints(bool bUpdateSpline = true);

	/** Adds an FSCCameraSplinePoint to the spline. This contains its input key, position, tangent, rotation and scale. */
	UFUNCTION(BlueprintCallable, Category = Spline)
	void AddPoint(const FSCCameraSplinePoint& Point, bool bUpdateSpline = true);

	/** Adds an array of FSCCameraSplinePoint to the spline. */
	UFUNCTION(BlueprintCallable, Category = Spline)
	void AddPoints(const TArray<FSCCameraSplinePoint>& Points, bool bUpdateSpline = true);
	...
	/** Get the Field of View at spline point */
	UFUNCTION(BlueprintCallable, Category = Spline)
	float GetFOVAtSplinePoint(int32 PointIndex) const;

	/** Get the Hold Time at spline point */
	UFUNCTION(BlueprintCallable, Category = Spline)
	float GetHoldTimeAtSplinePoint(int32 PointIndex) const;

	/** Get the Time to Next at spline point */
	UFUNCTION(BlueprintCallable, Category = Spline)
	float GetTimeToNextAtSplinePoint(int32 PointIndex) const;
	...
	/** Given a distance along the length of this spline, return the spline's Field of View there. */
	UFUNCTION(BlueprintCallable, Category = Spline)
	float GetFOVAtDistanceAlongSpline(float Distance) const;

	/** Given a distance along the length of this spline, return the spline's Hold Time there. */
	UFUNCTION(BlueprintCallable, Category = Spline)
	float GetHoldTimeAtDistanceAlongSpline(float Distance) const;

	/** Given a distance along the length of this spline, return the spline's Time to Next there. */
	UFUNCTION(BlueprintCallable, Category = Spline)
	float GetTimeToNextAtDistanceAlongSpline(float Distance) const;
	...
	/** Given a time from 0 to the spline duration, return the spline's Field of View there. */
	UFUNCTION(BlueprintCallable, Category = Spline)
	float GetFOVAtTime(float Time) const;

	/** Given a time from 0 to the spline duration, return the spline's Hold Time there. */
	UFUNCTION(BlueprintCallable, Category = Spline)
	float GetHoldTimeAtTime(float Time) const;

	/** Given a time from 0 to the spline duration, return the spline's Time to Next there. */
	UFUNCTION(BlueprintCallable, Category = Spline)
	float GetTimeToNextAtTime(float Time) const;

	UFUNCTION(BlueprintCallable, Category = Spline)
	FSCCameraSplinePoint GetSplinePointAtTime(float Time, ESplineCoordinateSpace::Type CoordinateSpace);
...
inline void USCCameraSplineComponent::AddSplineWorldPoint(const FVector& Position) { AddSplinePoint(Position, ESplineCoordinateSpace::World); }
inline void USCCameraSplineComponent::AddSplineLocalPoint(const FVector& Position) { AddSplinePoint(Position, ESplineCoordinateSpace::Local); }
inline void USCCameraSplineComponent::SetSplineWorldPoints(const TArray<FVector>& Points) { SetSplinePoints(Points, ESplineCoordinateSpace::World); }
inline void USCCameraSplineComponent::SetSplineLocalPoints(const TArray<FVector>& Points) { SetSplinePoints(Points, ESplineCoordinateSpace::Local); }
inline void USCCameraSplineComponent::SetWorldLocationAtSplinePoint(int32 PointIndex, const FVector& InLocation) { SetLocationAtSplinePoint(PointIndex, InLocation, ESplineCoordinateSpace::World); }
inline FVector USCCameraSplineComponent::GetWorldLocationAtSplinePoint(int32 PointIndex) const { return GetLocationAtSplinePoint(PointIndex, ESplineCoordinateSpace::World); }
inline void USCCameraSplineComponent::GetLocalLocationAndTangentAtSplinePoint(int32 PointIndex, FVector& LocalLocation, FVector& LocalTangent) const { GetLocationAndTangentAtSplinePoint(PointIndex, LocalLocation, LocalTangent, ESplineCoordinateSpace::Local); }
inline FVector USCCameraSplineComponent::GetWorldLocationAtDistanceAlongSpline(float Distance) const { return GetLocationAtDistanceAlongSpline(Distance, ESplineCoordinateSpace::World); }
inline FVector USCCameraSplineComponent::GetWorldDirectionAtDistanceAlongSpline(float Distance) const { return GetDirectionAtDistanceAlongSpline(Distance, ESplineCoordinateSpace::World); }
inline FVector USCCameraSplineComponent::GetWorldTangentAtDistanceAlongSpline(float Distance) const { return GetTangentAtDistanceAlongSpline(Distance, ESplineCoordinateSpace::World); }
inline FRotator USCCameraSplineComponent::GetWorldRotationAtDistanceAlongSpline(float Distance) const { return GetRotationAtDistanceAlongSpline(Distance, ESplineCoordinateSpace::World); }
inline FVector USCCameraSplineComponent::GetWorldLocationAtTime(float Time, bool bUseConstantVelocity) const { return GetLocationAtTime(Time, ESplineCoordinateSpace::World, bUseConstantVelocity); }
inline FVector USCCameraSplineComponent::GetWorldDirectionAtTime(float Time, bool bUseConstantVelocity) const { return GetDirectionAtTime(Time, ESplineCoordinateSpace::World, bUseConstantVelocity); }
inline FRotator USCCameraSplineComponent::GetWorldRotationAtTime(float Time, bool bUseConstantVelocity) const { return GetRotationAtTime(Time, ESplineCoordinateSpace::World, bUseConstantVelocity); }
