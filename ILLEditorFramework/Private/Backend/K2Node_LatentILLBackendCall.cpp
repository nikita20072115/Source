...
#include "K2Node_LatentILLBackendCall.h"
#include "ILLLatentBlueprintCallProxyBase.h"

#define LOCTEXT_NAMESPACE "ILLFonic.IGF.K2Node_LatentILLBackendCall"

UK2Node_LatentILLBackendCall::UK2Node_LatentILLBackendCall(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
	ProxyActivateFunctionName = GET_FUNCTION_NAME_CHECKED(UILLLatentBlueprintCallProxyBase, Activate);
}

void UK2Node_LatentILLBackendCall::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
	...
	for (TObjectIterator<UClass> ClassIt; ClassIt; ++ClassIt)
	{
		...
		if (!Class->IsChildOf<UILLLatentBlueprintCallProxyBase>() || Class->HasAnyClassFlags(CLASS_Abstract))
		{
			continue;
		}
		...
		for (TFieldIterator<UFunction> FuncIt(Class, EFieldIteratorFlags::ExcludeSuper); FuncIt; ++FuncIt)
		{
			...
			// see if the function is a static factory method for backend proxies
			bool const bIsProxyFactoryMethod = (ReturnProperty != nullptr) && ReturnProperty->PropertyClass->IsChildOf<UILLLatentBlueprintCallProxyBase>();
			...
			if (bIsProxyFactoryMethod)
			{
				...
				{
					UK2Node_LatentILLBackendCall* AsyncTaskNode = CastChecked<UK2Node_LatentILLBackendCall>(NewNode);
