...
static const FName PoweredPointLightGlow_NAME(TEXT("Glow"));

UPoweredPointLightComponent::UPoweredPointLightComponent(const FObjectInitializer& ObjectInitializer)
...
void UPoweredPointLightComponent::BeginPlay()
{
	Super::BeginPlay();

	RegisterWithPowerPlant();

	OnActivateDelegate.AddDynamic(this, &UPoweredPointLightComponent::OnActivate);
	OnDeactivateDelegate.AddDynamic(this, &UPoweredPointLightComponent::OnDeactivate);
	...
	if (!IsRunningDedicatedServer())
	{
		...
		for (USceneComponent* Child : Children)
		{
			if (UMeshComponent* MeshComponent = Cast<UMeshComponent>(Child))
			{
				...
				{
					if (Materials[i] && Materials[i]->GetScalarParameterValue(PoweredPointLightGlow_NAME, Val))
					{
			...
			if (UParticleSystemComponent* ParticleComponent = Cast<UParticleSystemComponent>(Child))
			{
				...
				for (int32 i(0); i<ParticleComponent->GetNumMaterials(); ++i)
				{
					...
					if (MI && MI->GetScalarParameterValue(PoweredPointLightGlow_NAME, Val))
					{
...
void UPoweredPointLightComponent::TickComponent(float DeltaSeconds, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	{
		DynamicMat->SetScalarParameterValue(PoweredPointLightGlow_NAME, NewGlow);
	}
...
void UPoweredPointLightComponent::OnVisibilityChanged()
{
	...
	for (UMaterialInstanceDynamic* DynamicMat : GlowMaterials)
	{
		DynamicMat->SetScalarParameterValue(PoweredPointLightGlow_NAME, GlowValue);
	}
}

void UPoweredPointLightComponent::SetPowered(bool bHasPower, UCurveFloat* Curve)
{
...
void UPoweredPointLightComponent::OnDeactivate()
{
	SetVisibility(false);
}

void UPoweredPointLightComponent::OnActivate()
{
	SetVisibility(true);
}
