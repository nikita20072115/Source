...
#include "PoweredSpotLight.h"

#include "PoweredPointLightComponent.h"
#include "PoweredSpotLightComponent.h"

static const FName PoweredActorSpotLightGlow_NAME(TEXT("Glow"));

APoweredSpotLight::APoweredSpotLight(const FObjectInitializer& ObjectInitializer)
...
void APoweredSpotLight::BeginPlay()
{
	Super::BeginPlay();
	...
	if (!IsRunningDedicatedServer())
	{
		...
		for (USceneComponent* Child : ChildrenComponents)
		{
			if (UMeshComponent* MeshComponent = Cast<UMeshComponent>(Child))
			{
				...
				{
					if (Materials[i] && Materials[i]->GetScalarParameterValue(PoweredActorSpotLightGlow_NAME, Val))
					{
			...
			if (UParticleSystemComponent* ParticleComponent = Cast<UParticleSystemComponent>(Child))
			{
				...
				for (int32 i(0); i<ParticleComponent->GetNumMaterials(); ++i)
				{
					...
					if (MI && MI->GetScalarParameterValue(PoweredActorSpotLightGlow_NAME, Val))
					{
...
void APoweredSpotLight::Tick(float DeltaSeconds)
{
	...
	{
		DynamicMat->SetScalarParameterValue(PoweredActorSpotLightGlow_NAME, NewGlow);
	}
...
void APoweredSpotLight::SetVisibility(bool bVisible)
{
	...
	for (UMaterialInstanceDynamic* DynamicMat : GlowMaterials)
	{
		DynamicMat->SetScalarParameterValue(PoweredActorSpotLightGlow_NAME, GlowValue);
	}
}

void APoweredSpotLight::SetPowered(bool bHasPower, UCurveFloat* Curve)
{
...
void APoweredSpotLight::OnActivate_Implementation()
{
	SetVisibility(true);
}

void APoweredSpotLight::OnDeactivate_Implementation()
{
	SetVisibility(false);
}
